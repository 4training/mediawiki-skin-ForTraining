<?php
/**
 * BaseTemplate class for the ForTraining skin
 *
 * @ingroup Skins
 */
...
class ForTrainingTemplate extends BaseTemplate {
	...
	protected function getLogo( $id = 'p-logo', $part = 'both' ) {
		...
		if ( $part !== 'image' ) {
			$wordmarkImage = $this->getLogoImage( $config->get( 'ForTrainingWordmark' ), true );

			$titleClass = '';
			$siteTitle = '';
			if ( !$wordmarkImage ) {
				if ( $language->hasVariants() ) {
					$siteTitle = $language->convert( $this->getMsg( 'fortraining-sitetitle' )->escaped() );
				} else {
					$siteTitle = $this->getMsg( 'fortraining-sitetitle' )->escaped();
				}
		...
		if ( $part !== 'text' ) {
			$logoImage = $this->getLogoImage( $config->get( 'ForTrainingLogo' ) );

			$html .= Html::rawElement(
				'a',
				array_merge(
					[
						'class' => [ 'mw-wiki-logo', !$logoImage ? 'fallback' : 'fortraining-logo' ],
						'href' => $this->data['nav_urls']['mainpage']['href']
					],
	...
	protected function getPageToolSidebar() {
		...
		$pageTools .= $this->getPortlet(
			...
			'fortraining-pageactions'
		);
		$pageTools .= $this->getPortlet(
			'userpagetools',
			$this->pileOfTools['user'],
			'fortraining-userpagetools'
		);
		$pageTools .= $this->getPortlet(
			'pagemisc',
			$this->pileOfTools['page-tertiary'],
			'fortraining-pagemisc'
		);
		...
		return $this->getSidebarChunk( 'page-tools', 'fortraining-pageactions', $pageTools );
	}
	...
	protected function getUserLinks() {
		...
		if ( isset( $personalTools['userpage'] ) ) {
			$personalTools['userpage']['links'][0]['text'] = $this->getMsg( 'fortraining-userpage' )->text();
		}
		if ( isset( $personalTools['mytalk'] ) ) {
			$personalTools['mytalk']['links'][0]['text'] = $this->getMsg( 'fortraining-talkpage' )->text();
		}

		// Labels
		if ( $user->isLoggedIn() ) {
			$dropdownHeader = $userName;
			$headerMsg = [ 'fortraining-loggedinas', $userName ];
		} else {
			$dropdownHeader = $this->getMsg( 'fortraining-anonymous' )->text();
			$headerMsg = 'fortraining-notloggedin';
		}
	...
	protected function getPageTools() {
		...
		if ( $namespace >= 0 ) {
			$pileOfTools['pagelog'] = [
				'text' => $this->getMsg( 'fortraining-pagelog' )->text(),
		...
		// Mobile toggles
		$pileOfTools['more'] = [
			'text' => $this->getMsg( 'fortraining-more' )->text(),
			'id' => 'ca-more',
		...
		if ( $this->sidebar['LANGUAGES'] !== false || $sortedPileOfTools['variants']
			...
			$pileOfTools['languages'] = [
				'text' => $this->getMsg( 'fortraining-languages' )->escaped(),
				'id' => 'ca-languages',
				'class' => 'dropdown-toggle'
			];
		}
